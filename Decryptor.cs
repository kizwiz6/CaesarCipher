...
namespace CaesarCipher
{
    internal class Decryptor
    {
        ...
        public char[] Decrypt(char[] encryptedMessage)
        {
            return Transform(encryptedMessage, -3);
        }

        private char[] Transform(char[] message, int key)
        {
            ...
            for (int i = 0; i < message.Length; i++)
            {
                ...
                if (char.IsLetter(letter))
                {
                    ...
                    transformedMessage[i] = letterDecoded;
                }
                else
                {
